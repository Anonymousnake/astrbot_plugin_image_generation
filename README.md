...
| 命令                                    | 说明                                                                |
| :-------------------------------------- | :------------------------------------------------------------------ |
| `/生图 <提示词、预设名称或人设名称> [额外提示词]` | 生成图片。消息、引用消息、@ 用户或人设参考图会作为参考图。 |
| `/生图模型`                             | 查看可用模型和当前模型。                                            |
| `/生图模型 <序号>`                      | 切换到指定序号的模型。                                              |
| `/预设`                                 | 查看所有预设和人设。                                                |
| `/预设 添加 <预设名:预设内容>`          | 添加预设。                                                          |
...
| 最大并发生图任务数   | 3                    | 同时运行的生图任务数量上限。                                    |
| 默认图片宽高比       | 自动                 | 可选自动、1:1、2:3、3:2、3:4、4:3、4:5、5:4、9:16、16:9、21:9。 |
| 默认分辨率           | 1K                   | 可选 1K、2K、4K；仅部分适配器支持。                             |
| 显示生成信息         | 关闭                 | 发送图片后附加耗时和图片数量。                                  |
| 显示模型信息         | 关闭                 | 发送图片后附加所用模型。                                        |
| 开始生图任务提示模板 | 已开始生图任务       | 生图开始时发送的提示文本；留空则不发送。                        |

开始生图任务提示模板支持这些占位符：`{reference_image_count}`、`{prompt}`、`{preset}`、`{aspect_ratio}`、`{resolution}`、`{task_id}`、`{model}`、`{mode}`、`{reference_images_block}`、`{preset_block}`。其中 `{preset}` 可表示命中的预设或人设。
...
`/预设` 会同时展示预设和人设。预设与人设同名时，`/生图` 优先使用预设。LLM 生图工具可通过 `preset` 参数使用预设，也可通过 `persona` 参数使用人设；当填写人设且未指定比例时默认使用 `9:16`，并支持 `prompt`、`aspect_ratio`、`resolution`、`avatar_references` 和 `reference_images` 参数。LLM 预设查询工具默认只返回预设或人设名称列表，也可以通过名称查看具体内容。LLM 预设编辑工具可创建和删除预设，创建时支持简单提示词格式和高级 JSON 格式。
