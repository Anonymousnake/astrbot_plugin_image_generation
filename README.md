...
| `start_task_message_template` | string  | `"已开始生图任务{reference_images_block}{preset_block}"` | 开始生图任务提示模板，留空则不发送提示。支持占位符：`{reference_image_count}`、`{prompt}`、`{preset}`、`{aspect_ratio}`、`{resolution}`、`{task_id}`、`{model}`、`{mode}`、`{reference_images_block}`、`{preset_block}`。 |

| 提示模板占位符             | 说明                                                                                                |
| :------------------------- | :-------------------------------------------------------------------------------------------------- |
| `{reference_image_count}`  | 本次请求提取到的参考图数量，例如 `2`。                                                              |
| `{prompt}`                 | 最终发送给生图模型的提示词。使用预设时，会显示展开后的完整提示词及额外追加内容。                    |
| `{preset}`                 | 命中的预设名称；未命中预设或通过 LLM Tool 自然语言调用时为空。                                      |
| `{aspect_ratio}`           | 本次请求使用的宽高比，例如 `1:1`、`16:9`、`自动`。                                                  |
| `{resolution}`             | 本次请求使用的分辨率，例如 `1K`、`2K`、`4K`。                                                       |
| `{task_id}`                | 本次生图任务 ID。                                                                                   |
| `{model}`                  | 当前使用的生图模型，格式为 `供应商名称/模型名称`。                                                  |
| `{mode}`                   | 根据参考图数量自动显示 `文生图` 或 `图生图`。                                                       |
| `{reference_images_block}` | 参考图数量文本块；有参考图时如 `[2张参考图]`，没有参考图时为空。不会发送原图、图片 URL 或文件路径。 |
| `{preset_block}`           | 预设文本块；命中预设时如 `[预设: 手办化]`，未命中时为空。                                           |
