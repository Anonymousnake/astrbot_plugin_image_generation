...
| 命令                                              | 说明                                                       |
| :------------------------------------------------ | :--------------------------------------------------------- |
| `/生图 <预设/人设...> [额外提示词]` 或 `/生图 <提示词>` | 生成图片。开头连续命中的预设/人设会按空格分隔并依次应用；消息、引用消息、@ 用户或人设参考图会作为参考图。 |
...
| 最大并发生图请求数   | 3                    | 同时调用图像模型接口的请求数量上限；多图任务中的每张图也按请求级别受此限制。                    |
| 默认出图数量         | 1                    | 未在指令或 LLM 工具中指定数量时，单个生图任务默认生成的图片数量。                               |
...
`/生图` 指令和 LLM 生图工具共用统一任务体系：任务提交后会生成 `task_id`，进入排队/运行/完成/失败/取消状态；模型接口调用受最大并发生图请求数限制。`/生图任务` 只展示当前会话正在进行的任务，并为列表项添加编号；可通过 `/生图任务 <编号或任务ID>` 查看详情，通过 `/生图取消 <编号或任务ID>` 取消仍在排队或运行中的任务。任务记录保存在内存中，插件重载后会清空；已结束任务不会在列表中展示。
