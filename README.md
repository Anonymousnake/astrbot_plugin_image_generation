...
> [!warning]
> 人设添加功能请使用 AstrBot `4.25.3` 以上的版本
...
LLM 生图工具可通过 `preset` 参数使用预设，也可通过 `persona` 参数使用人设，多个名称可用空格分隔；并支持 `prompt`、`aspect_ratio`、`resolution`、`avatar_references` 和 `reference_images` 参数。  
LLM 生图工具会在提交任务前去除重复参考图。参考图会校验真实图片类型；本地路径仅允许当前会话 workspace、AstrBot temp 目录和本插件数据目录。  
LLM 预设查询工具默认只返回预设或人设名称列表，也可以通过名称查看具体内容。  
LLM 预设编辑工具可创建和删除预设，创建时支持简单提示词格式和高级 JSON 格式。  
